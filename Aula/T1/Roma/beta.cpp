...
void RemoveRepeated(vector<pair<string,vector<string>>>&LocalFile)
{
  ...
  for(size_t i=0; i< LocalFile.size(); i++)//Percorre os files
  {
    ...
    {
      for(size_t k=0;k<LocalFile.size();k++)
      {
       for(size_t l=0;l<LocalFile.at(k).second.size();l++)
       {
         if(k==i && j== l)
         {
           cout<<"Igual: "<<LocalFile.at(k).second.at(l)<<endl;
         }
        else if(LocalFile.at(k).second.at(l)==LocalFile.at(i).second.at(j))
        {
          cout<<"Removido:"<<LocalFile.at(k).first<<" -> "<<LocalFile.at(k).second.at(l)<<" Pq e igual a "<<
          LocalFile.at(i).first<<" "<<LocalFile.at(i).second.at(j)<<endl;
  ...
  if(find=true)
  cout << "All repeated words removed" << endl;
  else
