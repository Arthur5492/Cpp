...
void RemoveRepeated(vector<pair<string,vector<string>>>&LocalFile)
{
  vector<pair<string,string>>>corn;
  ofstream filewritter;
  filewritter.open("temporary.txt"); //abre um novo arquivo

  for(size_t i=0; i<LocalFile.size(); i++){
     for(size_t j=0; j<LocalFile.at(i).second.size(); j++){
     if(LocalFile.at(i).second.at(j).size() != 0){
     filewritter <<i <<" -> "<< LocalFile.at(i).second.at(j) << endl;
   }
 } //passa as palavras dos arquivos linha por linha
     cout << endl;
  }
  string tempfile = "temporary.txt";
  ifstream filereader(tempfile);
    string tmp;
