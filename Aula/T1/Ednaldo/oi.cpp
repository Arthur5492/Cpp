  ...
  vector<pair<string, vector<string>>> ListofFiles;   // vetor contendo file por file
    while(true) //loop principal
    {
        ...
        if(ch == '1')
        {
          ...
          cin>>file;
          LoadDatabase(file,ListofStrings);
          files.first = file;
...
/*void RemoveRepeated(vector<string>&LocalFile)
{

for(int i = 0; i < LocalFile.size(); i++) {
  for(int j = 0;j = LocalFile.at(i).second.size();j++)
  {

    LocalFile.erase(LocalFile.begin()+j);
    }
  }
}
