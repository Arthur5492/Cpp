...
//Remove as palavras repitidas do ListofFiles
void RemoveRepeated(vector<pair<string,vector<string>>>&LocalFile)
{
  for(size_t i=0; i< LocalFile.size(); i++)//Percorre os files
  {
    ...
    {
      for(size_t k=0;k<LocalFile.size();k++)//Entra novamente na lista de arquivos
      {
       for(size_t l=0;l<LocalFile.at(k).second.size();l++)//Entra novamente nas palavras
       //Esses dois novos for, são para checar se a palavra é repetida em todos os files
       {
         if(k==i && j==l);//Para não apagar a própria palavra

        else if(LocalFile.at(k).second.at(l)==LocalFile.at(i).second.at(j))
        //Se a palavra que está sendo analisada for igual a palavra que está passando no for, apague a palavra do for
          LocalFile.at(k).second.erase(LocalFile.at(k).second.begin()+l);

      }
     }
    }
  }
  cout << "There are no repeated words no more" << endl;
}
